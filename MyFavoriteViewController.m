...
-(void)download{
    ...
    NSLog(@"%@",uid);
    [manager GET:LOVE parameters:@{@"uid":uid,@"like":guanzhu,@"lat":lat,@"lng":lng} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        NSDictionary *dic= [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"%@",dic);
        
        NSArray *shops = [dic objectForKey:@"shops"];
        NSLog(@"149行：%d",shops.count);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"212行：%d",_datasource.count);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"263行：%d",_datasource.count);
    loveModel *model =_datasource[indexPath.row];
